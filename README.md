<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=200&section=header&text=Anshita%20Dash&fontSize=60&fontColor=ffffff&animation=fadeIn&fontAlignY=35&desc=Full-Stack%20SDE%20%7C%20AI%2FML%20Layered%20Specialization&descAlignY=55&descSize=18" width="100%"/>

<a href="https://git.io/typing-svg">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&pause=1000&color=A78BFA&center=true&vCenter=true&width=600&lines=B.Tech+CSE+(AI%2FML)+%40+SOA+ITER;Building+Full-Stack+Projects+from+Scratch;UI%2FUX+Lead+%40+SIH+2026;Currently+Deep+in+DSA+%2B+Java" alt="Typing SVG" />
</a>

<br/>

![Location](https://img.shields.io/badge/📍_Location-Bhubaneswar,_India-6D28D9?style=flat-square)
![Academic](https://img.shields.io/badge/🎓_B.Tech-CSE_(AI%2FML)-4C1D95?style=flat-square)

<br/>

[![Portfolio](https://img.shields.io/badge/Portfolio-6D28D9?style=for-the-badge&logo=vercel&logoColor=white)](#)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-4C1D95?style=for-the-badge&logo=linkedin&logoColor=white)](#)
[![Email](https://img.shields.io/badge/Email-5B21B6?style=for-the-badge&logo=gmail&logoColor=white)](#)
[![GitHub](https://img.shields.io/badge/GitHub-1E1B4B?style=for-the-badge&logo=github&logoColor=white)](https://github.com/anshitadash98-maker)

<br/>

![Profile Views](https://komarev.com/ghpvc/?username=anshitadash98-maker&color=6D28D9&style=flat-square&label=Profile+Views)
![Followers](https://img.shields.io/github/followers/anshitadash98-maker?color=8B5CF6&style=flat-square&label=Followers)
![Stars](https://img.shields.io/github/stars/anshitadash98-maker?color=A78BFA&style=flat-square&label=Stars)

</div>

<br/>

## 🧬 About Me

```yaml
role: "Aspiring Full-Stack Software Engineer"
current_focus: "Building production-grade projects end-to-end"
specialization: "AI/ML as a layered capability, not the whole stack"
currently: "First-Year B.Tech CSE (AI/ML), SOA ITER, Bhubaneswar"
```

I'm an engineering student building real, deployed products instead of waiting for a classroom to hand me a project. My goal is a backend-leaning full-stack SDE role, with AI/ML layered on top as a specialization rather than a crutch. I care about actually understanding what my code does copy-pasting AI output without comprehension isn't part of how I work.



**🔭 Open to:** Internship opportunities in full-stack development, and collaborative student-led technical projects.

<br/>

# 📊 GitHub Stats:
![](https://github-readme-stats.shion.dev/api?username=anshitadash98-maker&theme=gruvbox_light&hide_border=false&include_all_commits=false&count_private=false)<br/>
![](https://streak-stats.demolab.com/?user=anshitadash98-maker&theme=gruvbox_light&hide_border=false)<br/>
![](https://github-readme-stats.shion.dev/api/top-langs/?username=anshitadash98-maker&theme=gruvbox_light&hide_border=false&include_all_commits=false&count_private=false&layout=compact)

---
[![](https://komarev.com/ghpvc/?username=anshitadash98-maker&icon=0&color=0)](https://visitcount.itsvg.in)



## 🛠️ Tech Stack

<div align="center">

**Languages**
<br/>
<img src="https://skillicons.dev/icons?i=java,python,js,ts,html,css" />

**Frontend**
<br/>
<img src="https://skillicons.dev/icons?i=react,vite,tailwind" />

**Backend & Databases**
<br/>
<img src="https://skillicons.dev/icons?i=flask,nodejs,postgres" />

**Tooling & Deployment**
<br/>
<img src="https://skillicons.dev/icons?i=git,github,vscode,vercel,npm" />

</div>

<br/>

## 🤖 AI/ML Exposure

| Domain | Proficiency | Details |
|---|---|---|
| Sentiment Analysis | Applied | Integrated VADER sentiment scoring into a full-stack mood-tracking app |
| API-driven data apps | Applied | Built on real-time external data (NASA NEO API) |
| Model deployment constraints | Learned the hard way | Hit memory limits running DistilBERT on free-tier hosting — pivoted to a lighter model and shipped it anyway |
| Core ML fundamentals | In progress | Building foundation alongside DSA and systems coursework |

<br/>

## 🚀 Featured Projects

<details>
<summary><b>🚌 TransitWatch — Real-Time Public Transport Tracking</b></summary>
<br/>

Simulated live bus GPS tracking system for small-city public transport, built as part of SIH 2026 (PS25013).

| | |
|---|---|
| **Stack** | React, Vite, Tailwind CSS, Leaflet |
| **Scale** | Multi-route live map simulation |
| **Focus** | Real-time UI updates, map-based UX |
| **Repository** | [View on GitHub](https://github.com/anshitadash98-maker) |

</details>

<details>
<summary><b>📓 Mood Journal — Sentiment-Aware Journaling App</b></summary>
<br/>

A journaling app that scores entries using sentiment analysis, deployed as a live full-stack product.

| | |
|---|---|
| **Stack** | Flask, VADER (NLP), Python |
| **Deployment** | Render |
| **Challenge overcome** | Redesigned model choice after hitting memory limits with DistilBERT on free-tier hosting |
| **Repository** | [View on GitHub](https://github.com/anshitadash98-maker) |

</details>

<details>
<summary><b>🛰️ NEO Tracker — Near-Earth Object Dashboard</b></summary>
<br/>

A space-themed dashboard consuming NASA's public API to visualize near-Earth object data.

| | |
|---|---|
| **Stack** | JavaScript, NASA Open API |
| **Deployment** | GitHub Pages |
| **Repository** | [View on GitHub](https://github.com/anshitadash98-maker) |

</details>

<details>
<summary><b>💧 India Water Watch — Groundwater Dashboard</b></summary>
<br/>

A multi-page groundwater data dashboard with scroll-triggered animations, built entirely in vanilla web technologies.

| | |
|---|---|
| **Stack** | HTML, CSS, JavaScript |
| **Deployment** | GitHub Pages |
| **Repository** | [View on GitHub](https://github.com/anshitadash98-maker) |

</details>













## 🎯 Current Focus

```yaml
learning:
  - Data Structures & Algorithms (Java)
  - Algorithm Analysis & Design
  - Machine Learning fundamentals

building:
  - Real-time transport tracking system 
  - Personal developer portfolio

exploring:
  - Backend architecture patterns
  - Applied AI/ML for full-stack products

open_to:
  - Internships in full-stack development
  - Student-led technical collaborations
```

<br/>

## 📫 Connect

<div align="center">

[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](#)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](#)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/anshitadash98-maker)
[![Portfolio](https://img.shields.io/badge/Portfolio-6D28D9?style=for-the-badge&logo=vercel&logoColor=white)](#)

</div>

<br/>

<div align="center">

*"Understand it, build it, ship it — in that order."*

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=100&section=footer" width="100%"/>

</div>
